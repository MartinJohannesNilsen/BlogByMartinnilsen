...
import StyledControlledSelect, {
  SelectOption,
} from "@/components/DesignLibrary/Select/StyledControlledSelect";
...
export const SettingsModal = (props: SettingsModalProps) => {
  ...
  >([]);
  const [themeUserConfigurationExist, setThemeUserConfigurationExist] =
    useState<boolean>();
            ...
            >
              {defaultFonts.map((element, index) => (
                <SelectOption value={element.font} key={index}>
                  <Typography
                    sx={{
                      fontFamily: getFontFamilyFromVariable(element.font),
                      fontWeight: 400,
                      fontSize: "0.875rem",
                    }}
                  >
              ...
              <Box display="flex" alignItems="center">
                <Box onClick={() => setColorPickerOpen(false)}>
                  ...
                  >
                    {defaultColors.map(
                      (
                        element: { title: string; color?: string },
                        index: number
                      ) => (
