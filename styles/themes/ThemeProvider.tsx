...
export const ThemeContext = createContext<ThemeContextType>({
  ...
  setDefaultTheme: () => {},
  accentColor:
    (typeof window !== "undefined" && localStorage.getItem("accent")) ||
    getSelectedTheme() === "dark"
  ...
  fontFamily:
    typeof window !== "undefined"
      ? getFontFamilyFromVariable(
          localStorage.getItem("fontFamily") || defaultFontFamilyVariable
        )
      : defaultFontFamily,
  setFontFamily: (fontFamily) => {},
  fontScale:
    (typeof window !== "undefined" && localStorage.getItem("fontScale")) ||
    defaultFontScale,
...
export const CustomThemeProvider = ({ children }: CustomThemeProviderProps) => {
  const OS_STANDARD = useMediaQuery(COLOR_SCHEME_QUERY, { noSsr: true })
    ? "dark"
    : "light";
  const [fontFamily, setFontFamily] = useStickyState(
    "fontFamily",
    defaultFontFamilyVariable
  );
  ...
  const [fontScale, setFontScale] = useStickyState(
    "fontScale",
    defaultFontScale
  );
  ...
  useEffect(() => {
    if (
      theme.palette.mode === "dark" &&
      accentColor == defaultAccentColorLight.hex
    ) {
      setAccentColor(defaultAccentColorDark.hex);
    } else if (
      theme.palette.mode == "light" &&
      accentColor == defaultAccentColorDark.hex
    ) {
  ...
  useMemo(() => {
    setTheme(
      theme.palette.mode === "dark" ? ThemeEnum.Dark : ThemeEnum.Light,
      false
    );
    return () => {};
