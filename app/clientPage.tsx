...
const LandingPage = ({
  ...
  useEffect(() => {
    if (postsOverview) {
      Promise.resolve(
        _filterListOfStoredPostsOnPublished(postsOverview, "published")
      ).then((allPosts) => {
        // Apply filters
        if (filters.startDate) {
          allPosts = allPosts.filter(
            (post) => new Date(post.createdAt) >= new Date(filters.startDate!)
          );
        }
        if (filters.endDate) {
          allPosts = allPosts.filter(
            (post) => new Date(post.createdAt) <= new Date(filters.endDate!)
          );
        }
        ...
        if (filters.minReadTime) {
          allPosts = allPosts.filter(
            (post) =>
              ...
              parseInt(post.readTime) >= filters.minReadTime!
          );
        }
        ...
        if (filters.maxReadTime) {
          allPosts = allPosts.filter(
            (post) =>
              post.readTime !== undefined &&
              parseInt(post.readTime) <= filters.maxReadTime!
          );
        }
        ...
        const newChunkedPosts = splitChunks(
          allPosts,
          Number(
            process.env.NEXT_PUBLIC_LANDING_PAGE_POSTS_PER_PAGE_GRID_LAYOUT
          )
        );
  ...
  const handleChangeView = (
    ...
    if (["carousel", "swipe", "grid", "list", "plain"].includes(newView))
        ...
        <Box height="100%">
          ...
          {cardLayout === "carousel" ? (
            <Box
              ...
              // height={"100vh"}
              height={
                xs && isMobile ? "calc(100vh - 88px)" : "calc(100vh - 114px)"
              }
              display="flex"
              flexDirection="column"
              justifyContent="center"
            ...
            <Box
              ...
              height={
                xs && isMobile ? "calc(100vh - 160px)" : "calc(100vh - 114px)"
              }
              display="flex"
              flexDirection="column"
              justifyContent="center"
