...
import {
  getViewCount,
  incrementViewCount,
} from "@/data/middleware/views/actions";
...
export function getNextRelevantPosts(
  ...
  const filteredPosts = allPosts.filter(
    (post) => post.id !== currentPostId && post.published
  );
...
export const ReadPostPage = ({
  ...
  useEffect(() => {
    ...
    if (postsOverview)
      setNextRelevantPosts(
        getNextRelevantPosts(postId, post, postsOverview, 3)
      );
  ...
  if (isMobile && isAuthorized) {
    buttonBarButtons.push({
      ...
      text: "",
      onClick: () => {
        window.location.href = `/cms/edit/${postId}`;
      },
    });
  }
  ...
  useGSAP(
    () => {
      ...
      let distanceToTrigger = parseInt(
        process.env.NEXT_PUBLIC_HIDE_NAVBAR_DISTANCE_IN_PX!
      );
      ...
      window.addEventListener("scroll", function () {
        ...
        if (scrollDirection === -1 && scrollDistance >= distanceToTrigger) {
          // Scrolling upwards
          if (
            (process.env.NEXT_PUBLIC_HIDE_NAVBAR_DESKTOP === "true" &&
              !isMobile) ||
            (process.env.NEXT_PUBLIC_HIDE_NAVBAR_MOBILE === "true" && isMobile)
          ) {
          ...
          scrollDirection === 1 &&
          scrollDistance >= distanceToTrigger
        ) {
          // Scrolling downwards
          if (
            (process.env.NEXT_PUBLIC_HIDE_NAVBAR_DESKTOP === "true" &&
    ...
    <Box width="100%" height="100%" position="relative" className="page">
      ...
      >
        ...
        <PostNavbar
          ...
          simpleTextModal={{
            open: openSimpleTextModal,
            setOpen: setOpenSimpleTextModal,
          }}
                        ...
                        onClick={() => {
                          isMobile
                            ? handleSharing({
                                ...
                                icon:
                                  post.ogImage.src ||
                                  DATA_DEFAULTS.images.openGraph,
                  ...
                  >
                    <Suspense fallback={<></>}>
                      ...
                      {discussionData.locked === true && (
                        ...
                        >
                          The comment section has been deactivated for this
                          post.
                        </Typography>
                      )}
                    </Suspense>
