...
import { Box, Typography, useMediaQuery, Modal, styled, IconButton, Alert } from "@mui/material";
...
export const FilterModal = (props: FilterModalProps) => {
	...
	const [startDate, setStartDate] = useState<string>(props.initialFilters.startDate || "");
	const [endDate, setEndDate] = useState<string>(props.initialFilters.endDate || "");
	const [minReadTime, setMinReadTime] = useState<number | null>(props.initialFilters.minReadTime || null);
	const [maxReadTime, setMaxReadTime] = useState<number | null>(props.initialFilters.maxReadTime || null);
	const [tags, setTags] = useState<string[]>(props.initialFilters.tags || []);
	const [sortOrder, setSortOrder] = useState<string>(props.initialFilters.sortOrder || "desc");
	...
	const handleApplyFilters = (e: any) => {
		...
		if (minReadTime !== null && maxReadTime !== null && minReadTime > maxReadTime) {
	...
	const handleMinReadTimeChange = (e: any) => {
		setWarning(null);
		let value = e.target.value === "" ? null : Math.max(1, parseInt(e.target.value));
		if (isNaN(value!)) {
			value = null;
		}
	...
	const selectStyles = {
		...
		option: (baseStyles, state) => ({
			...baseStyles,
			backgroundColor: state.isFocused ? theme.palette.action.hover : theme.palette.primary.main,
			color: theme.palette.text.primary,
		}),
		input: (baseStyles) => ({
			...baseStyles,
			...
			<Box component="form" onSubmit={handleApplyFilters} sx={style}>
				...
				<Box display="flex" gap="5px" alignItems="center" mt={1}>
					...
					<StyledInput
						...
						sx={{ width: "200px" }}
					/>
					<IconButton aria-label="clear min read time" size="small" onClick={handleClearStartDate}>
				...
				<Box display="flex" gap="5px" alignItems="center" mt={1}>
					...
					<Box flexGrow="1" />
					<StyledInput type="datetime-local" value={endDate} onChange={handleEndDateChange} sx={{ width: "200px" }} />
					<IconButton aria-label="clear min read time" size="small" onClick={handleClearEndDate}>
						<Close sx={{ fontSize: "1rem", color: theme.palette.text.primary }} />
					</IconButton>
				</Box>
				...
				<Box display="flex" gap="5px" alignItems="center" mt={1}>
					...
					<IconButton aria-label="clear min read time" size="small" onClick={handleClearMinReadTime}>
						<Close sx={{ fontSize: "1rem", color: theme.palette.text.primary }} />
					</IconButton>
				</Box>
				...
				<Box display="flex" gap="5px" alignItems="center" mt={1}>
					...
					<IconButton aria-label="clear max read time" size="small" onClick={handleClearMaxReadTime}>
						<Close sx={{ fontSize: "1rem", color: theme.palette.text.primary }} />
					</IconButton>
				</Box>
